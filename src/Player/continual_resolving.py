...
class ContinualResolving():
	...
	def start_new_hand(self, card1, card2, player_is_small_blind):
		...
		start_cfvs = self.starting_opponent_cfvs_as_P1 if self.player_position == P2 else self.starting_opponent_cfvs_as_P2
	...
	def compute_action(self, board_string, player_bet, opponent_bet):
		...
		if self.prev_street != node.street and self.prev_street+1 == node.street:
			assert(not node.terminal)
	...
	def _create_node(self, board_string, player_bet, opponent_bet):
		sb, bb, P1, P2 = arguments.sb, arguments.bb, constants.players.P1, constants.players.P2
		node = Node()
	...
	def _resolve_first_node(self):
		...
		results = self.first_node_resolving.resolve(first_node, player_range, opponent_range=opponent_range)
		# returns the initial CFVs [b, P, I], here b = 1
		self.starting_opponent_cfvs_as_P1 = results.root_cfvs_both_players[0,P1,:] # 0, because batches = 1
		self.starting_opponent_cfvs_as_P2 = results.root_cfvs_both_players[0,P2,:] # 0, because batches = 1
